...
name: spec
description: >
  Convierte un pedido en criollo en el contrato de lo que se va a hacer: requisitos numerados
  `Rn` y una verificación chequeable por requisito. Sirve para dos dominios: **código** (el paso
  previo al build) y **entregable** (un mail, una propuesta, un análisis, un reporte). Se informa
  primero, leyendo el código o la fuente de cada dato, y solo pregunta lo que la fuente no puede
  responder, siempre proponiendo la opción que asume. No construye nada. Usalo con "armá el spec
  de X", "definamos bien esto antes de tocar código", "escribí el contrato de este cambio", "antes
  de escribirlo definamos qué tiene que decir", "contra qué lo vamos a chequear". NO es para
  decidir entre opciones (eso es `council`), ni para revisar algo ya hecho (eso es `panel` o
  `verificar`), ni para trabajo exploratorio donde descubrir *es* el trabajo, ni para un fix de una
  línea, donde el spec cuesta más que el cambio.
---
