...
name: publicar
description: >
  Decís /brain-ship y te sube lo último que te hizo Claude (una página, una app), verifica
  que la URL abra y te devuelve el link. La primera vez, si no tenés dónde hostear, te hace
  crear la cuenta (Netlify o Vercel para páginas, Supabase si necesita base de datos), guarda
  la conexión y te la deja lista; de la segunda vez en adelante sube solo, sin preguntar.
  Usalo cuando digas "subí esto", "poné esto online", "publicá esto", "deployá", "quiero el
  link", "/brain-ship". Directo: sube, verifica y da el link, no te hace preguntas de más.
---
